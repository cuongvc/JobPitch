...
module.exports = elmongo = function (schema, options) {
    ...
    schema.statics.search = function (searchOpts, cb) {
        ...
        return helpers.doSearchAndNormalizeResults(searchUri, searchOpts, cb, 'search')
    }

    schema.statics.aggregateCount = function (searchOpts, cb) {
        options = helpers.mergeModelOptions(options, this)

        var searchUri = helpers.makeTypeUri(options) + '/_search?search_type=count&preference=_primary_first'
        if(!searchOpts){
            searchOpts = {};
        }
        return helpers.doSearchAndNormalizeResults(searchUri, searchOpts, cb, 'aggregate')
    }
