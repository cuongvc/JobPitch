var send_mail			=	require('./../../my_module/send_mail/send_mail_forgot_password');
...
module.exports			=	function(req, res){
	try{
		...
		res.write(JSON.stringify({error_code : 201, msg : err.toString()}));
		res.status(200).end();
	}
	finally{
		...
		Users.findOne({email : email}, function(err, user_exist){
			if (user_exist){
				...
				res.write(JSON.stringify({error_code : 200, msg :'User is not exist'}));
				res.status(200).end();
			}
		});
	}
